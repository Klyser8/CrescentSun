...
public class TimedSoundEffect extends SoundEffect {
    ...
    public void playForPlayerAtTheirLocation(Player player) {
        Bukkit.getScheduler().runTaskLater(plugin, () -> super.playForPlayerAtTheirLocation(player), getDelayInTicks());
    }

    @Override
    public void playForPlayerAtLocation(Player player, Location location) {
        Bukkit.getScheduler().runTaskLater(plugin, () -> super.playForPlayerAtLocation(player, location), getDelayInTicks());
    }
